...
func Lighting(m Material, l Light, pos, eyeV, normalV Tuple, inShadow bool) (Color, error) {
        ...
        if inShadow {
                ...
                final := ambient.Add(diffuse).Add(specular)
                final = Color{R: Clamp(final.R, 0, 1), G: Clamp(final.G, 0, 1), B: Clamp(final.B, 0, 1)}
                return final, nil
        }
}
